...
public class JinjaCallNode extends JinjaNode {
    ...
    /** Prints the callee followed by its call arguments, e.g. {@code loop.cycle(a, b)}. */
    @Override
    public void print(String indent) {
        System.out.println(indent + selfDescription());
        String base = TreePrinter.continuation(indent);
        TreePrinter.fields(base,
                (ind, last) -> TreePrinter.child(ind, last, "Callee", callee),
                (ind, last) -> TreePrinter.children(ind, last, "Arguments", arguments));
    }
}
