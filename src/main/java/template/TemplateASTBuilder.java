...
public class TemplateASTBuilder extends TemplateParserBaseVisitor<Object> {
    ...
    private void emit(Object item, Deque<Frame> stack, List<Object> topLevel) {
        if (item instanceof HtmlNode htmlNode) {
            // HTML nodes always land at the top level (see this method's
            // class-level Javadoc), regardless of which Jinja2 control-flow
            // frame, if any, is currently open. When an {% if %} frame IS
            // open, additionally record a JinjaHtmlRefNode in whichever
            // branch is currently active, so Generator can later tell -
            // once it knows which branch the condition actually selects -
            // that this HTML node belonged only to a branch that was not
            // taken. See JinjaHtmlRefNode for the full reasoning; FOR/BLOCK/
            // MACRO frames deliberately do not get this treatment, since
            // repeating or gating a whole HTML subtree (as opposed to
            // excluding it outright) is a larger change than this fixes.
            topLevel.add(htmlNode);
            if (!stack.isEmpty() && stack.peek().kind == Frame.Kind.IF && !isInsignificantWhitespace(htmlNode)) {
                // Whitespace-only text (the newline-and-indentation between
                // "{% if %}" and the tag on the next line, for instance) is
                // real, harmless output either way, so tracking it here would
                // only add noise to the printed tree and to
                // Generator.suppressedHtmlNodes for no visible benefit.
        ...
        if (ctx.htmlContent() != null) {
            for (Object item : collectContentItems(ctx.htmlContent())) {
                if (item instanceof HtmlNode htmlNode) {
                    ...
                    // it resolved to right here - instead of only ever being
