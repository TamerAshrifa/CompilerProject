 ...
 * Jinja2 trees are built independently: a Jinja construct that textually sat
 * inside an HTML tag's content was hoisted out to the program's top-level
 * Jinja list while parsing (see {@code TemplateASTBuilder}'s
 * {@code hoistedJinjaNodes}), so this class — walking only
 * {@code getHtmlElements()} — reproduces the HTML skeleton faithfully but,
 * by design of the AST it reads, cannot re-interleave the Jinja output back
 * into it. Combining the two into one final interleaved document is a
 * pipeline-level concern for a later stage (see {@link CodeGenerator}).
 *
...
public class HtmlGenerator extends TemplateBaseVisitor<String> {
    ...
    public String visitHtmlElement(HtmlElementNode node) {
        ...
        if (children.size() == 1 && children.get(0) instanceof HtmlTextNode) {
            sb.append('>').append(children.get(0).accept(this)).append("</").append(tagName).append('>');
            return sb.toString();
        }
