...
public class JinjaGenerator extends TemplateBaseVisitor<String> {
    ...
    private String renderJinjaOperand(JinjaNode operand, int minimumPrecedence) {
        if (operand instanceof LiteralNode literal && "string".equals(literal.getLiteralType())) {
            // A quoted string literal is always atomic; there is nothing left
    ...
    /**
     * Renders {@code node} as a value nested inside a larger Jinja2
     * expression — a call argument, a subscript index, or anything else that
     * (unlike {@link #renderJinjaOperand}'s callers) doesn't need parenthesis
     * handling but still sits inside an expression rather than at a
     * top-level output position. This differs from a plain
     * {@code node.accept(this)} in exactly one case: a string
     * {@link LiteralNode} parsed directly from Jinja2 source syntax (e.g. the
     * {@code 'Hello, '} in {@code {{ 'Hello, ' + name }}}) must be re-quoted
     * here to stay valid Jinja2 syntax when regenerated.
     *
     * <p>This is deliberately <em>not</em> done inside {@link
     * #visitJinjaLiteral} itself, because the same {@link LiteralNode} class
     * also represents an already-resolved substitution value produced by
     * {@link Generator} (e.g. {@code {{ name }}} &rarr; {@code Ada}), which
     * always sits at a top-level statement/output position — never nested
     * inside another expression, since {@link Generator} only ever replaces
     * a <em>whole</em> {@code {{ ... }}} unit — and must print bare, with no
     * quotes, so it reads as final output text rather than Jinja2 source.
     * Whether a given {@code LiteralNode} needs quoting therefore depends on
     * where it sits in the tree, not on the node in isolation; every call
     * site that visits a genuinely <em>nested</em> node (as opposed to a
     * top-level statement/body position, which should keep using plain
     * {@code accept(this)}) should route through this method instead.</p>
     */
    private String renderJinjaValue(JinjaNode node) {
        if (node instanceof LiteralNode literal && "string".equals(literal.getLiteralType())) {
            support.mark(literal.getNodeName(), literal.getLine(), literal.getColumn());
            return "'" + escapeJinjaString(literal.getStringValue()) + "'";
        }
        return node.accept(this);
    }

    /** Escapes {@code value} for safe embedding inside a single-quoted Jinja2 string literal. */
    private String escapeJinjaString(String value) {
