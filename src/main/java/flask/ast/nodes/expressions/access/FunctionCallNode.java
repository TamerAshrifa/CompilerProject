...
public class FunctionCallNode extends Expression {
    ...
    /**
     * Prints the callee, positional and keyword arguments, and - only when
     * actually used - the {@code *args}/{@code **kwargs} call-site spreads.
     */
    @Override
    public void print(String indent) {
        System.out.println(indent + selfDescription());
        List<TreePrinter.Field> fields = new ArrayList<>();
        fields.add((ind, last) -> TreePrinter.child(ind, last, "Callee", callee));
        fields.add((ind, last) -> TreePrinter.children(ind, last, "Args", args));
        fields.add((ind, last) -> TreePrinter.entries(ind, last, "Kwargs", kwargs));
        if (!starArgs.isEmpty()) {
            fields.add((ind, last) -> TreePrinter.children(ind, last, "StarArgs", starArgs));
        }
        if (hasKwargsSpread()) {
            fields.add((ind, last) -> TreePrinter.children(ind, last, "KwargsSpread", kwargsSpread));
        }
