...
public class FullPipelineIntegrationTest {
    ...
    private static void runComplexRealisticTemplateIntegrationTest() {
        ...
        // The if/else branches themselves are still present in the Jinja
        // output even though is_profitable could not be resolved to a
        // literal by the existing Context/TemplateTransformer (it only
        // resolves simple variable references, not boolean conditions) -
        // this is existing, unmodified behavior from before this stage,
        // confirmed still true here.
        if (!result.getGeneratedJinjaSource().contains("{% if")) {
            throw new AssertionError("Expected the conditional structure to still be present in the Jinja output");
        }
