...
public class CodeGeneratorTest {
    ...
    /**
     * Documents, deliberately and explicitly, the honest boundary described in
     * {@link FinalDocumentGenerator}'s class Javadoc: a Jinja construct that was
     * hoisted out from several levels deep inside a wrapping HTML element (the
     * realistic case — a real page's content normally sits inside
     * {@code <html><body>...}) ends up placed after that whole top-level
     * element, not re-nested at its original depth, because nothing in the
     * existing AST records where an element's content ends. This is not an
     * oversight: re-nesting it would require either guessing at an unrecorded
     * span, duplicating HtmlGenerator's own tag-rendering logic elsewhere, or
     * redesigning HtmlGenerator's rendering model — each of which trades away
     * one of this task's own explicit rules (no shortcuts; no duplicated
     * code; do not rewrite previous work). What this test asserts is exactly
     * what IS still true in that case: the resolved value is present, the
     * surrounding structure is present, nothing is silently dropped or
     * fabricated — just not re-interleaved at its original nesting depth.
     */
    private static void runFinalDocumentGeneratorNestedScopeLimitationTest() {
        ...
        int htmlEnd = merged.indexOf("</html>");
        int titleIdx = merged.indexOf("{{ title }}");
        if (!(htmlEnd >= 0 && titleIdx > htmlEnd)) {
            throw new AssertionError(
                    "Expected the documented behavior (hoisted content placed after the whole top-level "
                            + "element it was found under, not re-nested inside it), got:\n" + merged);
        }
     ...
     * (both via the existing, untouched {@code Generator}/{@code TemplateTransformer}),
     * generated, and merged into one Final HTML Document with CSS injected.
     */
